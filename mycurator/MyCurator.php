            ...
            foreach ($topics as $topic) {
             ...
             if (!empty($sources) && !empty($topics) && $cnxions && !$testmode) {
                 ...
                 echo "<p><img src='$imggood' ></img> Your setup is ready to go.  You have ".$tstr.' and '.$sstr.'. '
                         ...
                         . "when articles are posted.</p>";
                 echo "<p>If you are using the <strong>Gutenberg Editor</strong>, "
                 . "make sure the Option Create Gutenberg Blocks into Curated Posts? is Set. "
                         . "If you are using the <strong>Classic Editor</strong> make sure it is clear!</p>";
                 $dostatus = true;
                ...
                if (isset($_POST['Wizard']) ) {
                    ...
                    if (!$error_flag) {
                        ...
                        $args['rss-url'] = 'https://www.bing.com/news/search?q='.$srchterm.'&format=rss'; //Bing Search string
                 ...
                 if (empty($mct_ai_optarray['ai_on'])) {
                    ...
                    //If cron is past its schdule time + 5 minuts, something is wrong
                        ...
                        . " try refreshing this page a couple of times over the course of a few minutes to see if the background process will start. "
                                ...
                                . " <a href='https://www.siteground.com/tutorials/wordpress/real-cron-job/' >Getting Cron to run on time</a></li>";
...
function mct_ai_optionpage() {
   ...
   $args = array(
    ...
    if (isset($_POST['Submit']) ) {
        ...
        //Clean up domains
        $valid_str = '';
        $varray = $opt_update['ai_skip_domains'];
        $varray = str_replace(" ","\n",$varray); //change spaces to line breaks
        $varray = explode("\n",$varray);
        foreach ($varray as $vstr){
            $vstr = trim(sanitize_text_field($vstr));
        ...
        $opt_update['ai_skip_domains'] = trim($valid_str);
        ...
        <div class="mct-ai-tabs" >
         <div id="tabs">
            ...
            <div id="tabs-1">            
                ...
                <tr>
                    <th scope="row">Create Gutenberg Blocks into Curated Posts?</th>
                    <td><input name="ai_save_guten" type="checkbox" id="ai_save_guten" value="1" <?php checked('1', $cur_options['ai_save_guten']); ?>  /></td>    
                </tr>
            ...
            <div id="tabs-2">
                ...
                <tr>
                    <th scope="row">Skip These Domains</th>
                    <td><textarea id='ai_skip_domains' rows='5' cols='100' name='ai_skip_domains'><?php echo $cur_options['ai_skip_domains'];  ?></textarea>
                    <span>&nbsp;<em>One Domain per Line</em></span></td>    
                </tr>
                <tr>
                    <th scope="row">Insert Image Only when Made Live?</th>
                    <td><input name="ai_only_live_img" type="checkbox" id="ai_only_live_img" value="1" <?php checked('1', $cur_options['ai_only_live_img']); ?>  /></td>     
                </tr> 
                </table>
            </div>
            <div id="tabs-3">
            ...
            <div id="tabs-5" >
               ...
               <tr><th><strong>Administrative Settings</strong></th>
                ...
                <tr>
                    <th scope="row">Feed Cleaner Base URL</th>
...
function mct_ai_setoptions($default) {
    ...
    $opt_update = array (
            ...
            'ai_save_guten' => ($default) ? 1 : absint((isset($_POST['ai_save_guten']) ? $_POST['ai_save_guten'] : 0)),
            'ai_feed_cleaner' => ($default) ? '' : trim(sanitize_text_field($_POST['ai_feed_cleaner'])),
            'ai_skip_domains' => ($default) ? '' : $_POST['ai_skip_domains'], //validation in options page
            'ai_only_live_img' => ($default) ? 0 : absint((isset($_POST['ai_only_live_img']) ? $_POST['ai_only_live_img'] : 0)),
...
u=f+'&u='+e(l.protocol+'//'+l.host+l.pathname+l.search)+'&t='+e(d.title)+'&s='+e(s)+'&v=4';
...
function mct_ai_logspage() {
    ...
    if (isset($_REQUEST['Filter'])){
        check_admin_referer('mct_ai_logsspg','logfilter');
    ...
    if (isset($_REQUEST['Reset-Log'])){
        check_admin_referer('mct_ai_logsspg','logreset');
        mct_ai_clearlogs();
    }
    ...
    if ($search != '') $sql.= " AND locate('$search',concat(logs_msg,' ',logs_url))";
    ...
    <h2>MyCurator Logs</h2>    
     ...
     <p>You can reset the logs and the filter that keeps articles from being re-read.  If you do this, you will most likely
         ...
         <form id="Reset" method="post" >
             <?php wp_nonce_field('mct_ai_logsspg','logreset'); ?>
	...
	if ( $page_links )
        ...
        //Search field
        echo ' Search Message and URL Fields: <input name="search" type="text" id="search" size="20" maxlength="50" value="" />';
        wp_nonce_field('mct_ai_logsspg','logfilter');
...
function mct_ai_showplan($display=true, $upgrade=true){
    ...
    //if (!is_string($mct_ai_optarray['ai_plan'])) {
    //    mct_ai_log('Blog',MCT_AI_LOG_ERROR, 'Error - Invalid ai_plan, not a string',var_dump($mct_ai_optarray['ai_plan']));
    //    return false;
    //}
    $plan = maybe_unserialize($mct_ai_optarray['ai_plan']);
    if ($plan['max'] == -1) {
        ...
        if (mct_ai_cloudtopic($topic)) {
            ...
            if ($postit) {
                ...
                if (!empty($_REQUEST['publish']) || !empty($_REQUEST['publish-draft'])) {
                    ...
                    mct_nb_traintodraft($newpost,$postcat,$poststat,$topic['topic_type']);
                }
                return $newpost;
            }
        }
...
function mct_ai_dosourceit() {
    ...
    <div id="poststuff" class="metabox-holder">
            ...
            <div class="mct-posting">
                ...
                <br /> 
                <?php if ($msg != '' && $posted) { ?>
                        <div id="message" class="error" ><p><strong><?php echo "SOURCE NOT CREATED: ".$msg ; ?></strong></p>
                            <p><a href="#" onclick="window.close();"><?php _e('Click to Close Window'); ?></a></p></div>
                <?php 
                       if (stripos($msg,'valid') !== false) {
                           echo "<p>You will need to work with the site owner to fix an invalid feed.  Click above to get the details "
                           . "and you can send the link of that page to them to see if they can correct it.</p>";
                       }
                       exit(); } ?>
