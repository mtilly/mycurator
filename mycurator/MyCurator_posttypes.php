...
add_filter('post_row_actions','mct_ai_remove_quick_edit',20,1);
...
function mct_ai_register(){
    ...
    if ($mct_ai_optarray['MyC_version'] != MCT_AI_VERSION) {
        ...
        //
        //Version 3.7 Changes
        if (!isset($mct_ai_optarray['ai_only_live_img'])) $mct_ai_optarray['ai_only_live_img'] = 0;  //Save image only when made live
        //Version 3.6 Changes
        if (!isset($mct_ai_optarray['ai_feed_cleaner'])) $mct_ai_optarray['ai_feed_cleaner'] = '';
        if (!isset($mct_ai_optarray['ai_skip_domain'])) $mct_ai_optarray['ai_skip_domain'] = '';
        //Version 3.3 Changes
        if (!isset($mct_ai_optarray['ai_save_guten'])) $mct_ai_optarray['ai_save_guten'] = 0; //Default is not using Gutenberg blocks
        //Version 3.1 Changes
...
/* Exclude Target Post Type From Yoast SEO Sitemap
function mct_ai_exclude_post_type( $value, $post_type ) {
    if ( $post_type == 'target' ) return true;
}
add_filter( 'wpseo_sitemap_exclude_post_type', 'mct_ai_exclude_post_type', 10, 2 );
*/
...
function target_ai_shortcode(){
    ...
    if (isset($_GET['topic'])){
        $qtopic = sanitize_text_field($_GET['topic']);
        set_transient('mct_ai_lasttopic',$qtopic,60*60);
    }
    elseif (isset($_GET['ai_class'])){
        $qaiclass = sanitize_text_field($_GET['ai_class']);
...
function is_trainee($postid){
    ...
    if ($tname == '') return 'Filter';
     ...
     if (!is_array(get_post_meta($postid,'mct_sl_newurl',true))) return 'No';
...
function bwc_create_news(){
    ...
    if (isset($_POST['Submit'])){
        ...
        if ($feed_type == 'G') {
            ...
            $args['rss-url'] = 'https://www.bing.com/news/search?q='.$newsterm.'&format=rss'; //Bing Search string &qft=interval%3d%228%22+sortbydate%3d%221%22
...
function mct_ai_trainscript($page) {
    ...
    wp_enqueue_script('mct_ai_train',$jsdir,array('jquery','thickbox','jquery-ui-dialog'),'1.0.5');
