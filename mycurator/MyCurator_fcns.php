...
function mct_ai_postlink($args, $update = false){
    ...
    global $current_user, $mct_ai_optarray;
    ...
    $url = $args['rss-url'];
    //Validate a feed
    if (!$update AND stripos($url,'twitter.com') === false) {
        $Vurl = "https://validator.w3.org/feed/check.cgi?url=";  
        $Vurl = $Vurl.$url;
        
        $response = fetch_feed($url);

        if( is_wp_error( $response )) {
            $msg = 'Feed Not Valid, '.$response->get_error_message().' <a href="'.$Vurl.'" target="_blank">Click Here for Details</a>';
            if (!empty($mct_ai_optarray['ai_feed_cleaner'])) {
                $Curl = $mct_ai_optarray['ai_feed_cleaner'].rawurlencode($url);
                $response = fetch_feed($Curl);
                if( !is_wp_error( $response )) $msg = '';
            }
            if ($msg != '') return $msg;
        }
        unset($response);
	...
	"link_rss"		=> $url, // varchar, a URL of an associated RSS feed
...
function mct_ai_find_image($page) {
    //Find an image in the page passed in
    //Return structure of image and imgtitle
    $img_ret = array(
        'image' => '',
        'imgtitle' => ''
    );
    ...
    if ($pos) {
        ...
        $img_ret['image'] = $image;
        $img_ret['imgtitle'] = $imgtitle;
    }
    return $img_ret;
}

function mct_ai_post_image($post_id, $image, $imgtitle,$post_title,$ext_url) {
    //Post an image to thumbnail or insert in post
    global $mct_ai_optarray;
    ...
    if (!empty($mct_ai_optarray['ai_image_title'])) $imgtitle = $post_title;
    
    $thumb_id = mct_ai_postthumb($image,$post_id, $imgtitle);
    
    if ($thumb_id) {  
        if (!empty($mct_ai_optarray['ai_save_thumb'])) update_post_meta( $post_id, '_thumbnail_id', $thumb_id );
        //Add image to start of post if set
        if (!empty($mct_ai_optarray['ai_post_img']) ){
            $details = array();
...
function mct_ai_getplan($force = false){
    ...
    if (!$force && get_transient('mct_ai_getplan') == 'checked') { //In case we are doing a lot of work
        ...
        $plan = maybe_unserialize($mct_ai_optarray['ai_plan']);
        if (isset($plan['max'])) {
...
function mct_ai_train_ajax() {
    ...
    if (!empty($args['quick'])){ 
        ...
        //new text
        $newtext = '';
        if (!empty($notes)) $newtext = '<p id="mct-ai-quicknote">'.$notes.'</p>';
        if (!empty($excerpt)){
...
function mct_ai_traintoblog($thepost, $status){
    ...
    if (!empty($topic)) {
        ...
        $sql = "SELECT topic_cat, topic_tag, topic_tag_search2, topic_type, topic_options FROM $ai_topic_tbl WHERE topic_name = '$tname'";
        ...
        if (!empty($row)){
            ...
            //Do we need to add image if live only?
            if (!empty($mct_ai_optarray['ai_only_live_img'])) {
                if (!empty($mct_ai_optarray['ai_save_thumb'])  || !empty($mct_ai_optarray['ai_post_img'])) {
                    $post_content = get_post_field('post_content', $thepost);
                    if ($row['topic_type'] == 'Video' && stripos($post_content,'<iframe') !== false) return; //Add Image Only if no embedded video
                    //Do we already have an image (options may have been changed recently)?
                    $thumb_id = 0;
                    if (has_post_thumbnail( $thepost )) {
                        $thumb_id = get_post_meta($thepost, '_thumbnail_id',true);
                    } else {
                        $ids = $wpdb->get_col("SELECT ID FROM {$wpdb->posts} WHERE post_parent = $thepost AND post_type = 'attachment'");
                        $thumb_id = (!empty($ids)) ? $ids[0] : 0; //use only first one
                    }
                    if ($thumb_id == 0) { //No so lets look and post
                        $page = mct_ai_getslpage($thepost);
                        $img_ret = mct_ai_find_image($page);
                        $image = $img_ret['image'];
                        $imgtitle = $img_ret['imgtitle'];
                        $post_title = get_post_field('post_title', $thepost);
                        $url = '';
                        if (!empty($mct_ai_optarray['ai_title_link'])) {
                            $url = get_post_meta($thepost,'mct_sl_origurl',true);
                            $url = $url[0];
                        }
                        mct_ai_post_image($thepost, $image, $imgtitle,$post_title,$url);
                    }
                }
            }
...
function mct_ai_gutenblocks($content, $type){
    //This function changes the $content into Gutenberg blocks and returns $newcontent
    //$type is the Topic type, check it for video type
    global $mct_ai_optarray, $wp_version;
    if (empty($mct_ai_optarray['ai_save_guten'])) return $content;
    
    $newcontent = '';
    
    //Video topic
    if ($type == "Video" && stripos($content, '<iframe') !== false){
        $strtframe = stripos($content, '<iframe');
        //Build iframe block
        $pos = preg_match('{src="([^"]*)"}',$content,$matches);
        $src = $matches[1];
        $pos = preg_match('{width="([^"]*)"}',$content,$matches);
        $width = $matches[1];
        $pos = preg_match('{height="([^"]*)"}',$content,$matches);
        $height = $matches[1];
        $newcontent = '<!-- wp:html --><figure><iframe src="'.$src.'" width="'.$width.'" height="'.$height.'" '.'allowfullscreen="allowfullscreen"></iframe></figure><!-- /wp:html -->';
        
        //Build Description block
        $descstrt = stripos($content,'<p class="mct_ai_ytdesc">');
        if ($descstrt !== false) {
            $descend = stripos($content,'</p>',$descstrt);
            $desc = substr($content,$descstrt,$descend+3-$descstrt+1);
            $newcontent .= '<!-- wp:paragraph -->'.$desc.'<!-- /wp:paragraph -->';
        }
        //Look for quick text 
        $notestrt = stripos($content,'<p id="mct-ai-quicknote">');
        if ($notestrt !== false) {
            $notend = stripos($content,'</p>',$notestrt)+4; //content before excerpt
            $quicktext = substr($content,$notestrt, $notend - $notestrt); 
            $newcontent .= '<!-- wp:paragraph -->'.$quicktext.'<!-- /wp:paragraph -->';
        }
            
        //Build attribution link block
        $linkstrt = stripos($content,'<p id="mct-ai-attriblink">');
        if ($linkstrt !== false) {
            $linkend = stripos($content,'</p>',$linkstrt);
            $link = substr($content,$linkstrt,$linkend+3-$linkstrt+1);
            if (empty($mct_ai_optarray['ai_attr_top'])) {
                $newcontent .= '<!-- wp:paragraph -->'.$link.'<!-- /wp:paragraph -->';  //link at bottom
            } else {
                $newcontent = '<!-- wp:paragraph -->'.$link.'<!-- /wp:paragraph -->'.$newcontent; //link at top
            }
        }
        if ($newcontent == '') return $content; //Nothing found, just return as is
        return $newcontent;
    }
    //Non-Video Topic
    //Get Image Block
    $imgpos = stripos($content,'<img');  //Image actually there?
    if($imgpos !== false) {
        //Yes, so create image block
        //Get link
        $shortstr = substr($content,0,$imgpos); //Content up to start of image
        $linkstrt = strripos($shortstr, '<a href');  //Get last link in the short string
        $imglink = substr($shortstr,$linkstrt,$imgpos-$linkstrt);
        //Get src and alt
        $pos = preg_match('{src="([^"]*)"}',$content,$matches);
        $src = $matches[1];
        $pos = preg_match('{alt="([^"]*)"}',$content,$matches);
        $alt = $matches[1];
        $align = $mct_ai_optarray['ai_img_align'];
        if ($align == 'none') $align = 'left';  //set as default for blocks
    }
    //Get Attribution link
    $link = '';
    $linkstrt = stripos($content,'<p id="mct-ai-attriblink">');
    if ($linkstrt !== false) {
        $linkstrt = $linkstrt + 26;
        $linkend = stripos($content,'</p>',$linkstrt);
        $link = substr($content,$linkstrt,$linkend-$linkstrt);
    }
    //Get Excerpt
    $excerpt = '';
    $excstrt = stripos($content,'<blockquote id="mct_ai_excerpt">');
    if ($excstrt !== false) {
        $excend = stripos($content,'</blockquote>',$excstrt);
        $excstrt = $excstrt + 32;  //remove blockquote tag with ID in it
        $excerpt = substr($content,$excstrt,$excend-$excstrt);
    } else {
        $excstrt = stripos($content,'<p id="mct_ai_excerpt">');
        if ($excstrt !== false) {
            $excend = stripos($content,'</p>',$excstrt);
            $excstrt = $excstrt + 23;  //remove p tag with ID in it
            $excerpt = substr($content,$excstrt,$excend-$excstrt);
        }
    }
    if (substr($excerpt,0,3) == '<p>') { //Get rid of extra <p> tags due to line breaks
        $len = strlen($excerpt);
        $excerpt = substr($excerpt,3,$len-7);
    }
    $excerpt = html_entity_decode($excerpt, ENT_QUOTES | ENT_XML1, 'UTF-8');
    // Get Notes text from Quick
    $notes = '';
    $notestrt = stripos($content,'<p id="mct-ai-quicknote">');
    if ($notestrt !== false) {
        $notestrt = $notestrt + 25;
        $notend = stripos($content,'</p>',$notestrt); //content before excerpt
        $notes = substr($content,$notestrt, $notend - $notestrt); 
    }
    //Now lets build the Gutenberg blocks
    //Is image on Top?
    if (!empty($mct_ai_optarray['ai_post_img']) && empty($mct_ai_optarray['ai_image_bottom']) && $imgpos !== false) {
        $newcontent .= '<!-- wp:image {"align":"'.$align.'","linkDestination":"custom"} -->';
        $newcontent .= '<div class="wp-block-image"><figure class="align'.$align.'">';
        $newcontent .= $imglink.'<img src="'.$src.'" alt="'.$alt.'" /></a></figure></div>';
        $newcontent .= '<!-- /wp:image -->';
    }
    //Is Attribution Link on top?
    if (!empty($mct_ai_optarray['ai_attr_top']) && $link != ''){
        $newcontent .= '<!-- wp:paragraph --><p>'.$link.'</p><!-- /wp:paragraph -->';  //link after image
    }
    //Set Notes if we have them
    if ($notes != '') {
        //create paragraph block with the notes
        $newcontent .= '<!-- wp:paragraph --><p>'.$notes.'</p><!-- /wp:paragraph -->';
    }
    //Create quote or paragraph block with excerpt
    if (empty($mct_ai_optarray['ai_no_quotes'])) {
        $newcontent .= '<!-- wp:quote --><blockquote class="wp-block-quote"><p>'.$excerpt.'</p></blockquote><!-- /wp:quote -->';
    } else {
        $newcontent .= '<!-- wp:paragraph --><p>'.$excerpt.'</p><!-- /wp:paragraph -->';
    }
    //Attribute link on bottom?
    if (empty($mct_ai_optarray['ai_attr_top']) && $link != ''){
        $newcontent .= '<!-- wp:paragraph --><p>'.$link.'</p><!-- /wp:paragraph -->';  //link after image
    }
    //Image on the bottom?
