...
function mct_ai_rssapi($feed, $topic){
    ...
    if (is_wp_error($thefeed)){
        //Can we correct it?
        $Vurl = "https://validator.w3.org/feed/check.cgi?url=".$feed->link_rss;  //For error log
        $msg = 'Feed Not Valid, <a href="'.$Vurl.'" target="_blank">Click Here for Details</a>';
        if (!empty($mct_ai_optarray['ai_feed_cleaner'])) {
            $Curl = $mct_ai_optarray['ai_feed_cleaner'].rawurlencode($feed->link_rss);
            $thefeed = fetch_feed($Curl);
            if (!is_wp_error($thefeed)) $msg = '';
        } 
        if ($msg != '') {
            mct_ai_log($topic['topic_name'],MCT_AI_LOG_ERROR, $msg ,$feed->link_rss, $feed->link_name);
            //$thefeed->__destruct(); 
            unset ($thefeed);
...
function mct_ai_get_page($item, $topic, &$post_arr){
    global $wpdb, $blog_id, $ai_postsread_tbl, $ai_sl_pages_tbl, $mct_ai_stored_page_id, $mct_ai_current_link, $mct_ai_optarray;
    ...
    //Check if globally excluded domain
    if (!empty($mct_ai_optarray['ai_skip_domains'])) {
    ...
    if (!empty($topic['topic_skip_domains'])){
...
function mct_ai_setPRpage($page, $topic, $post_arr){
    ...
    if (!$page_id){
        //Bad data in page, insert without page
        if ($wpdb->last_error == '') {
            $val_array['pr_page_content'] = '';
            $wpdb->insert($ai_postsread_tbl, $val_array);
            $page_id = $wpdb->insert_id;
        }
        if (!$page_id){
            ...
            mct_ai_log($topic['topic_name'],MCT_AI_LOG_ERROR, 'Bad Data in Article, Could not Save Article Content in Postsread DB',$post_arr['current_link'], $post_arr['source']);
        }
        return '';
    }
}
...
function mct_ai_post_entry($topic, $post_arr, $page){
    ...
    $page_id = $wpdb->insert_id;
    if (!$page_id) {
        //save blank page if bad data in page (no error message)
        if ($wpdb->last_error == '') {
            $wpdb->insert($ai_sl_pages_tbl, array ('sl_page_content' => ''));
            $page_id = $wpdb->insert_id;
            if (!$page_id) {
                ...
                mct_ai_log($topic['topic_name'],MCT_AI_LOG_ERROR, 'Bad Data in Article, Could not Save Article Content',$post_arr['current_link'], $post_arr['source']);
            }
        } else {
    ...
    //Set Youtube thumbnail if Video and no other image
    if (empty($image) && $topic['topic_type'] == 'Video') {
        if (!empty($mct_ai_optarray['ai_video_thumb']) && !empty($post_arr['yt_thumb'])) {
        ...
        //update the image if found and meets live post criteria as the featured image or inserted image for the post 
        if (empty($mct_ai_optarray['ai_only_live_img']) OR (!empty($mct_ai_optarray['ai_only_live_img']) && $post_msg == "Live")) {
            mct_ai_post_image($post_id, $image, $imgtitle,$post_arr['title'],$post_arr['current_link']);
        }
    }
    //Save as gutenberg blocks if live (fcn does check of option)
    if ($post_msg == "Live") {
        $details = array();
        $details['post_content'] = mct_ai_gutenblocks(get_post_field('post_content', $post_id),$topic['topic_type']);
        $details['ID'] = $post_id;
