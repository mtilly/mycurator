...
//Show Page scripts insert
add_action('admin_print_styles-post.php', 'mct_ai_showpg_queue');
add_action('admin_print_styles-post-new.php', 'mct_ai_showpg_queue');
//add_action('admin_enqueue_scripts', 'mct_ai_showpg_queue');
...
function mct_nb_queuejs(){
    ...
    wp_enqueue_script('mct_nb_notebk',$jsdir,array('jquery','thickbox','jquery-ui-dialog'),'1.0.1');
...
function mct_nb_notepg_display($this_notebk) {
    ...
    <form id="np-bulk-action" method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
        ...
        <style>
        th.mct-np-title {width: 20%; }
        th.mct-np-note {width: 25%; }
        th.mct-np-date {width: 10%; }
        ...
        <table class="wp-list-table widefat" >
            <thead>
                <tr>
                    ...
                    <th class="mct-np-date">Date Added</th>
            ...
            <tbody>
                ...
                <tr class="post-<?php echo $notebk->ID; ?>" >
                    ...
                    <td><?php echo date("m-d-y",strtotime($notebk->post_date)); ?></td>
...
function mct_nb_traintodraft($thepost,$cat,$stat='draft',$type='filter'){
    ...
    $details['post_content'] = mct_ai_gutenblocks(get_post_field('post_content', $thepost),$type);  //function checks if we save blocks
    wp_update_post($details);
...
function mct_nb_choosenb($post){
    ...
    <p>Save or Update then Reload the Post using your Browser to Bring in Notebook Pages</p>
